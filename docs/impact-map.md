...
| `BookingForm` | `home/new-booking`, `home/quick-book` (2) | Form completo prenotazione. Legge `bookingSheetStore`. Prop `embedded`. Modificarlo cambia ENTRAMBE le route. Lista allievi = solo fase **PRATICA** (REG-499), filtrata in `IstruttoreHomeScreen.bookingStudentOptions` → cambia per ENTRAMBE. **Conferme del backend (REG-509)**: le risposte con `code` in `CONFIRMABLE_CODES` (`WEEKLY_LIMIT_CONFIRM`, `LESSON_BUFFER_CONFIRM`) NON sono errori — `runBooking` le ripropone con Annulla/Procedi e rimanda la conferma come flag, accumulandola alle precedenti. ⚠️ **Ogni nuovo `code` di conferma aggiunto lato server va aggiunto a quell'elenco**, altrimenti finisce nell'alert generico «Errore … OK» che ha un solo bottone e l'azione diventa impossibile da app (era il bug REG-509). |
