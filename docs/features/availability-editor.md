...
## What it does
Instructor availability management. The mode (`default` vs `publication`) is chosen in **Settings** (`more/availability-mode`), NOT inside the Disponibilità screen.
...
## Default mode — "Settimana tipo + Eccezioni"
- **Settimana tipo** (inline card): 7 weekday toggles (navy) + shared time ranges (`RangesEditor`) + "Salva settimana tipo" (navy). → `createAvailabilitySlots` (backwards-compat `startsAt2/endsAt2` for a 2nd range).
- **Eccezioni**: list of upcoming overrides (`getDailyAvailabilityOverrides`, future-only, sorted). Tap a row → edit/remove that date. "Aggiungi eccezione" → page-sheet:
...
Each range = two tappable **Inizio / Fine** cards (white, soft shadow — they're CTAs) showing a small uppercase label + large time; tap opens the time picker. `×` removes a range (when >1). "Aggiungi fascia" = a filled light button (subtle border, no dashed). Used by `publish-day`, `availability-exception`, and the default "Settimana tipo".
...
- `base` (default-mode weekly schedule) → "Settimana tipo" instant.
...
## API functions used
`getDefaultAvailability`, `getDailyAvailabilityOverrides`, `setDailyAvailabilityOverride`, `deleteDailyAvailabilityOverride`, `setRecurringAvailabilityOverride`, `createAvailabilitySlots`, `publishWeek`, `unpublishWeek`, `getPublishedWeeks`.
