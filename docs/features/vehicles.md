...
- **Auto al seguito** — its own row: the car name (tap → B-car picker with a "Nessuna auto al seguito" option, swaps/clears it) with a `×` to remove — **always available since 2026-07-03: the global rule suggests the follow car, it doesn't force it** (a `consigliata` tag shows when the rule is on). When none is set, a dashed **"Aggiungi auto al seguito"** row. `BookingForm` create path mirrors this: the follow-car picker offers "Nessuna" (sentinel `__none__` → null) and confirm requires an explicit choice.
...
Ogni veicolo serve **una** categoria patente (`B | BE | C | CE | D | DE | AM | A1 | A2 | A`, le non-moto BE/C/CE/D/DE aggiunte 2026-07-02: match esatto, nessuna gerarchia; la gerarchia esiste solo tra le moto) + un cambio (`manual | automatic`); ogni allievo in PRATICA persegue un percorso patente. Il matcher BE abbina l'allievo solo a un istruttore con un veicolo idoneo a categoria **e** cambio (istruttore moto = mai allievi auto). Tutta la logica è **gated su `vehiclesEnabled`** lato BE.
