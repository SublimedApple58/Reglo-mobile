...
**"Gestisci guida" multi-vehicle (2026-06-30):** `manage-lesson` shows a structured mono-navy **"Veicoli"** block when a moto guide involves more than one vehicle — the primary moto (editable, tap → vehicle picker), extra motos and the follow car, each with a moto/car icon + role label ("Moto principale" / "Moto aggiuntiva" / "Auto al seguito"). A single-vehicle guide keeps the one "Veicolo" row; the hero meta drops the long joined vehicle string when multi.
