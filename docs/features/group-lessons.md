...
### Visibilità lato ALLIEVO (segnalazione Robatto, 2026-09-02)
Prima l'allievo NON vedeva birilli/strada (solo istruttore/back-office). Ora sì, in tre punti: **inviti** (`GroupLessonInvitesScreen` → `GroupLessonInvite.motoLessonType`, esposto dal BE `getGroupLessonInvites`) per scegliere a quale iscriversi; **dettaglio** guida di gruppo (`app/(tabs)/home/group-lesson-detail.tsx` → `getGroupLesson().motoLessonType`, riga "Tipo guida"); **home** (`AllievoHomeScreen`, card guida prenotata → `AutoscuolaAppointment.groupLessonMotoType`, chip compatto). Icone/etichette da `src/utils/motoLessonType.ts`. **Guide moto INDIVIDUALI** (stesso giro): l'allievo vede birilli/strada in **home** (hero card della prossima guida, chip nel footer) e nel **dettaglio** (`app/(tabs)/home/lesson-detail.tsx`, riga "Tipo guida"). Il BE ha aggiunto `motoLessonType` al `select` del ramo `light` di `getAutoscuolaAppointmentsFiltered` (usato dalla home allievo). Le mini-card restano senza (compatte).
